 ...
 *
 * @var string $userName
 * @var string $totalAmount   e.g. "$450.00"
 * @var array  $charges       [{contractTitle, amount, hours}]
...
<p style="margin:0 0 20px;font-size:15px;color:#374151;line-height:1.6;">
    ...
    has been processed across
    <?= count($charges ?? []) ?> contract
    <?= count($charges ?? []) !== 1 ? 's' : '' ?>.
</p>
    ...
    <table width="100%" cellpadding="6" cellspacing="0" style="font-size:14px;color:#374151;">
        <thead>
            <tr style="border-bottom:2px solid #e5e7eb;">
                ...
                <th align="right" style="font-weight:600;color:#6b7280;padding-bottom:8px;">Amount</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach (($charges ?? []) as $charge): ?>
                <tr style="border-bottom:1px solid #f3f4f6;">
                    <td style="padding:8px 0;">
                        <?= htmlspecialchars($charge['contractTitle'] ?? '—') ?>
                    </td>
                    <td align="right" style="padding:8px 0;">
                        <?= htmlspecialchars($charge['hours'] ?? '—') ?>
                    </td>
                    <td align="right" style="font-weight:600;padding:8px 0;">
                        <?= htmlspecialchars($charge['amount'] ?? '$0.00') ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<p style="margin:0 0 20px;font-size:13px;color:#9ca3af;line-height:1.5;">
    All charges include platform fees. View your full billing history for detailed invoices.
</p>
