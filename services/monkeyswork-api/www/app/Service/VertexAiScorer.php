...
final class VertexAiScorer
{
    ...
    public function __construct()
    {
        ...
        $this->model = getenv('VERTEX_AI_MODEL') ?: 'gemini-2.5-flash';
    }
     ...
     * @return array{confidence: float, reasoning: string, checks: string[], model: string}
     */
    public function scoreVerification(string $type, array $data = []): array
    {
        if ($this->isSimulated()) {
    ...
    private function simulateJobScore(array $job): array
    {
        ...
        if ($budgetMax > 0 && $budgetMin > 0) {
            ...
            if ($budgetMin > $budgetMax) {
                $flags[] = 'budget_invalid';
                $quality -= 0.2;
            }
            if ($budgetMax > 1000000) {
                $flags[] = 'budget_suspicious';
                $quality -= 0.1;
            }
        }
    ...
    private function simulateVerificationScore(string $type): array
    {
        $baseScores = [
            'identity' => 0.88,
        ...
        $score = max(0.0, min(1.0, $base + $variance));

        return [
            'confidence' => round($score, 4),
            'reasoning' => "Simulated: {$type} scored {$score} (base {$base} ± variance)",
            'checks' => [],
            'model' => 'simulated-dev-v1.0',
        ];
    }
...
PROMPT;
    ...
    private function vertexVerificationScore(string $type, array $data): array
    {
...
4. Whether the data looks legitimate for a real freelancer
...
Respond in STRICT JSON format only, no markdown, no text before or after:
{
  ...
  "checks": ["<list of specific items you checked>"]
}
PROMPT;
