...
/**
 * useNotifications — Real-time notification feed via Socket.IO.
 *
 * Usage:
 *   const { notifications, unreadCount, markRead } = useNotifications(token);
 */
...
export function useNotifications(token?: string): UseNotificationsReturn {
    ...
    const markRead = useCallback(
        (notificationId: string) => {
            ...
            // Notify server (for analytics / sync)
