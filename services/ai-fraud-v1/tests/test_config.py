"""Tests for ai-fraud-v1 configuration."""
import os
import importlib
...
class TestSettingsDefaults:
    ...
    def test_default_service_name(self):
        ...
        s = Settings()
        assert s.service_name == "ai-fraud-v1"

    def test_default_environment_is_dev(self):
        from src.config import Settings
        s = Settings()
        # Default without ENVIRONMENT env var should be "dev"
    ...
    def test_settings_singleton_exists(self):
        ...
        assert settings.service_name == "ai-fraud-v1"


class TestSettingsEnvOverrides:
    """Test that env vars control Settings values.

    Since defaults use os.getenv() at class-definition time, we must
    reload the module after patching the environment.
    """
